...
Each model consisted of four chains with 2000 iterations, half of which were warm-up iterations, for a total of 4000 samples after warm-up. Langauge was coded as an index variable \citep{mcelreath_2020_sr}, such that a separate intercept was estimated for each of the two langauges. \hl{ Divergent transitions, R-hat, ESS}. These models give high-level insight into how much individuals differed for particular acoustic dimensions across language. The models also give insight into whether or not there is a common pattern across talkers. While this part of the analysis extends the work done primarily on F0 \citep[e.g.,][]{ng_2012_ltas} to the present data set, it does not elucidate how the different acoustic dimensions vary. % Prior work based mostly on ANOVA and t-tests
    
Inference in Bayesian statistics is based on the posterior distributions, which reflect the range and probability of credible values for parameters. In the case of comparing acoustic measurements across languages for the various talkers, the relevant comparison is the by-talker difference between langauges. While the overall effect of Language is also relevant and will be reported, the more interesting model result arises from the variability across slopes and intercepts. As such, results reporting for each of the 12 acoustic measurements will proceed as follows: posterior summaries for each language globally, and a discussion of how individual across-language differences vary. 

Reporting on the global effects is accomplished by reporting 95\% highest-density credible intervals (HDIs) for both levels of Language and their difference. Bayesian estimation is primarily focused on estimating the magnitude of parameters, and in this case, their difference. While using a descision criteria is not a requirement, it can aid interpretation. I used Kruschke's region of practical equivalence (ROPE) method with a threshold of XX \citep{kruschke_2011_rope}. In the case of the standardized measures (all but SHR), the ROPE is defined as [-0.1, 0.1]. In this decision framework, when 95\% of the posterior distribution falls above or below the ROPE, the effect is considered to be present. When more than 5\% the posterior overlaps with the ROPE, it would be considered a ``null'' effect. While Bayesians tend to shy away from using decision criteria, it nonetheless provides a useful scaffolding for interpreting the magnitudes of standardized effects. 

Individual differences are depicted visually, using forest plots of the difference between langauges by talker, accompanied by a prose description of what the variability means for interpretation of the variable. Figures XX depict the magnitude of the difference across languages for each of the different acoustic measurements. Each of the figures for the other acoustic measures can be similarly interpreted. Positive difference indicate that a talker produces a particular measure with higher Cantonese values. Negative difference values indicate that a talker produces the measure with lower Cantonese values compared to English. In each of the plots, the light blue shaded interval represents the ROPE. The posterior distribution is summarized by the mean (the point), and the 95\% HDI (the line)
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_f0_5in.png} 
%     \caption{Posterior difference between languages for F0.}
%     \label{ch3:fig:f0}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_f1_5in.png} 
%     \caption{Posterior difference between languages for F1.}
%     \label{ch3:fig:f1}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_f2_5in.png} 
%     \caption{Posterior difference between languages for F2.}
%     \label{ch3:fig:f2}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_f3_5in.png} 
%     \caption{Posterior difference between languages for F3.}
%     \label{ch3:fig:f3}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_f4_5in.png} 
%     \caption{Posterior difference between languages for F4.}
%     \label{ch3:fig:f4}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_h1h2_5in.png} 
%     \caption{Posterior difference between languages for H1*-H2*.}
%     \label{ch3:fig:h1h2}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_h2h4_5in.png} 
%     \caption{Posterior difference between languages for H2*-H4*.}
%     \label{ch3:fig:fh2h4}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_h4h2k_5in.png} 
%     \caption{Posterior difference between languages for H4*-H2kHz*.}
%     \label{ch3:fig:h4h2k}
%     \end{center}
% \end{figure}
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_h2kh5k_5in.png} 
%     \caption{Posterior difference between languages for H2kHz*-H5kHz*.}
%     \label{ch3:fig:h2kh5k}
%     \end{center}
% \end{figure}
...
% CPP
The model for CPP estimates similar posterior distributions for Cantonese () and English (), which is reflected by the small difference magnitude (). This difference overlaps substantially with the ROPE, indicating that globally-speaking, CPP does not differ across languages. On a by-talker level, however, this differs. Figure \ref{ch3:fig:cpp} depicts the posterior difference for CPP between languges for each talker. Higher CPP values are associated with both breathy or creaky non-modal phonation types. In this sense, a positive difference would indicate that Cantonese was more non-modal, while a negative difference would indicate that English was more non-modal. Interpreting CPP is not so straightforward, however, as it is not immediately clear which type of non-modal phonation is reflected in the measure. In any case, there is no consistent pattern across language for CPP. And considering that a majority of the comparison soverlap with the ROPE, the takeaway is that while individuals vary, the global effect is largely representative.
...
% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_energy_5in.png} 
%     \caption{Posterior difference between languages for Energy.}
%     \label{ch3:fig:energy}
%     \end{center}
% \end{figure}
...
In the case of SHR, the model looked a little bit different\dots 

% \begin{figure}[htbp]
%     \begin{center}
%     \includegraphics[width=\linewidth]{figures/ch3_shr_5in.png} 
%     \caption{Posterior difference between languages for SHR.}
%     \label{ch3:fig:shr}
%     \end{center}
% \end{figure}
