...
df %>%
  ...
  ggplot(aes(y=mean_vot, x=talker, color=language, label=segment)) +
    geom_text(fontface='bold', position=position_dodge(width = 0.33)) +
    geom_line(alpha=0.5, position=position_dodge(width = 0.33)) +
    scale_color_viridis_d(option = 'inferno', begin = 0.1, end = 0.8) +
    theme(
      axis.text = element_text(angle=90)
    )

# ggsave('ordrel.png', height=10, width = 5, units='in')
```
...
df %>%
  ...
  ggplot(aes(y=language, x=mean_vot, shape=segment, color=segment)) +
    geom_point() +
    geom_errorbar(position = "identity", width = .25, aes(xmin = mean_vot-se_vot, xmax = mean_vot+se_vot)) +
...
ggsave('ptk_vot_individuals2.png', height = 3, width = 8, units='in')
```
...
rdf %>%
  group_by(talker, language, segment) %>%
  summarise(mean_vot = mean(residual_vot_ms), se_vot = std.error(residual_vot_ms), n=n()) %>%
  ungroup() %>%
  ggplot(aes(y=language, x=mean_vot, shape=segment, color=segment)) +
    geom_point() +
    geom_errorbar(position = "identity", width = .25, aes(xmin = mean_vot-se_vot, xmax = mean_vot+se_vot)) +
    geom_line(aes(group=segment)) +
    scale_color_viridis_d(option =, begin = 0.1, end = 0.8) +
...
rdf %>%
  group_by(talker, segment, language) %>%
  summarise(mv = mean(residual_vot_ms)) %>%
  ungroup() %>%
  pivot_wider(names_from = 'segment', values_from = 'mv') %>%
  ungroup() %>%
  drop_na() %>%
