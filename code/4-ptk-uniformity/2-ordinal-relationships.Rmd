...
# ggsave('figs/ordrel.png', height=3.5, width = 12, units='in') # This used in ISBPAC poster
```
...
df %>%
  ...
  ggplot(aes(y=language, x=mean_vot, shape=segment, color=segment, group=segment)) +
    geom_point(shape=5, position = position_dodge(width = .5)) +
    geom_errorbar(position = position_dodge(width = .5), size=.5,width = .25, aes(xmin = mean_vot-se_vot, xmax = mean_vot+se_vot)) +
    geom_line(size=.75, position = position_dodge(width = .5),) +
    scale_color_viridis_d(option = 'inferno', begin = 0.1, end = 0.8) +
