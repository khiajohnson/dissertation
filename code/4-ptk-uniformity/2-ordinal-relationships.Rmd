...
df %>%
  ...
  ggplot(aes(y=mean_vot, x=language, color=segment, shape=segment)) +
  geom_point(size = 2.5 , position=position_dodge(width = 0.750)) +
    geom_errorbar(width=0.85, size=0.5 ,aes(ymin = lower, ymax = upper), position=position_dodge(width = 0.75)) +
    scale_color_viridis_d(option = 'magma', begin = 0, end = 0.7, breaks = c('P','T','K')) +
    scale_shape_discrete(breaks = c('P','T','K')) +
  ylab('Mean VOT (ms)') +
  xlab('') +
  coord_flip() +
  facet_wrap(~talker, ncol=3, scales='free_x') +
    theme(
