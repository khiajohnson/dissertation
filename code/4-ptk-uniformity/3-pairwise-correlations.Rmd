...
```{r fig.width=8}
cor.plot(output$r, show.legend = FALSE, pval=output$p.adj, stars =TRUE, diag=TRUE, upper=FALSE)
```
...
rdfout$ci %>%
  ...
  mutate(comparison = str_replace_all(comparison, 'r','')) %>%
  separate(comparison, into = c('a', 'b'), sep='-') %>%
  separate(a, into = c('a_lang', 'a_seg'), sep = -1) %>%
  separate(b, into = c('b_lang', 'b_seg'), sep = -1) %>%
  arrange(a_lang, b_lang, a_seg, b_seg) %>%
