...
ggpairs(dfsummary,lower=list(continuous=GGally::wrap("smooth", alpha=0.6, size=1)),
                  diag=list(continuous=GGally::wrap("densityDiag", alpha=0.5)),
                  upper=list(continuous=GGally::wrap("cor", method="pearson", stars=F, size=5))) + 
  xlab('Mean VOT') +
  ylab('Mean VOT') +
...
p <- grid.arrange(cpt, cpk, ctk, ept, epk, etk, cep, cet, cek, ncol=3)
ggsave('../../text/figures/ch4_correlations1_5in.png', plot=p, height = 6, width = 6, units='in')
```

```{r}
p <- grid.arrange(cpet, cpek, ctep, ctek, ckep, cket, ncol=3)
ggsave('../../text/figures/ch4_correlations2_5in.png',plot=p, height = 4, width = 6, units='in')
