...
```{r eval=FALSE}
cantodict <- read_tsv('/Users/khia/SpiCE/Dictionaries/canto20210305.dict', col_names = c('word', 'pronunciation'))

librespeech <- read_csv('/Users/khia/SpiCE/Dictionaries/librispeech-lexicon.dict', col_names = c('item'))
...
```{r}
interview_onset_times <- read_csv('/Users/khia/Nextcloud/SpiCE_Corpus/corpus_info/spice_interview_onset_timestamps.csv') %>%
  select(talker=speaker, language, interview_onset_s)
```
...
df <- df %>%
  ...
  select(talker, language, segment=avot_phone, vot_ms, is_post_pausal, following_vowel=next_phone, avg_phone_dur_ms, prev_word, word)
```
...
why are Cantonese /p/ counts lower?
```{r}
df %>%
