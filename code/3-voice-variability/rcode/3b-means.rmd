...
Subset and standardize for a particular acoustic measure
```{r}
subset_and_standardize <- function(fulldf, measure, samplesize) {
  
  subset <- mutate(measure = scale(measure)) %>%
            group_by(Talker, Language) %>%
            slice_sample(n = samplesize) %>%
            ungroup() %>%
            select(Talker, Language, measure)
...
By talker density plot
```{r}
plot_density_by_talker <- function(subsetdf, measure, x_label) {
  
  p <- measure %>%
    ggplot(aes(x=measure, color=Language)) +
    geom_density() +
    scale_color_viridis_d(option='magma', begin = 0.75, end = 0)+
...
Plot results as difference between Languages by Talker
```{r}
plot_difference_forest <- function(fit, y_label, rope_min=-0.1, rope_max=0.1) {
...
p <- fit %>%
  ...
  stat_pointinterval() + 
  annotate(geom = "rect", ymin = rope_min, ymax = rope_max, xmin = -Inf, xmax = Inf,
...
```{r fig.width=5, fig.height=6.5}
f0 %>%
  ggplot(aes(x=F0_z, color=Language)) +
    geom_density() +
    scale_color_viridis_d(option='magma', begin = 0.75, end = 0)+
    facet_wrap(~Talker, ncol = 5) +
    ylab('Density') +
...
```{r}
f0.fit.1 <- brm(data = f0, 
             family = student(),
             formula = F0_z ~ 0 + Language + (0 + Language | Talker),
