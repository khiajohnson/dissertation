...
```{r fig.width=12}
return_loadings_plot(filter(results, Talker=='VF19A', abs(Loading) > 0.32))
```

This is a bit messy, but its a useful structure for exploring the components.. a little better than it was
```{r}
results %>%
  filter(abs(Loading)>0.32) %>%
  select(Talker, Language, Variable, Variance, Var_Order) %>%
  pivot_wider(names_from = Variable, values_from = Variable) %>%
  select(Talker, Language, Variance, Var_Order, F0, F0_sd, F1, F1_sd, F2, F2_sd, F3, F3_sd, F4, F4_sd, H1H2c, H1H2c_sd, H2H4c, H2H4c_sd, H2KH5Kc, H2KH5Kc_sd, H42Kc, H42Kc_sd, CPP, CPP_sd, Energy, Energy_sd, SHR, SHR_sd) %>%
  group_by(Language, F0, F0_sd, F1, F1_sd, F2, F2_sd, F3, F3_sd, F4, F4_sd, H1H2c, H1H2c_sd, H2H4c, H2H4c_sd, H2KH5Kc, H2KH5Kc_sd, H42Kc, H42Kc_sd, CPP, CPP_sd, Energy, Energy_sd, SHR, SHR_sd) %>%
